...
function NoticeBoard(props) {
    ...
    const [addNoticeError, setAddNoticeError] = useState(false)
    ...
    function publishNotice(e) {
        e.preventDefault();
        if(subject === "" || message ===""){
            setAddNoticeError("Subject or message empty")
        }else {
        axios
    ...
    return (
        ...
        <div className={props.activeLink === "Home" || props.activeLink === "" ? "notice-container display" : "notice-container hide"}>
            ...
            <button type="button" className={showAddNoticeForm ? "hide":"display"} onClick={()=> {
                setShowAddNoticeForm(!showAddNoticeForm)
                console.log(showAddNoticeForm)
            }}>Add new notice</button>

            <form className={props.role  === "Admin" && showAddNoticeForm ? "create-new-notice display" : "create-new-notice hide"}>
                <div className={addNoticeError.length > 0 ? "error-message display" : "error-message hide"}>{addNoticeError}</div>
